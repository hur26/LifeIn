...
**摘要在微信里就能看,提问必须在企业微信里发。** 分开记:

| 方向 | 在哪 |
| --- | --- |
| **系统推给你**(每日摘要、提醒) | 关注了微信插件之后,**微信里直接收** —— 不用装企业微信也不用打开它 |
| **你发给系统**(提问) | **必须在企业微信 App 里发。** 微信那侧是只读的转投,不是对话入口 |

所以日常是这样:摘要在微信里扫一眼就完了;偶尔想问一句,才打开企业微信。

这正是 [ADR-001](04-tech-decisions.md#adr-001--推送与审批入口选企业微信)
选企微而不选飞书的核心理由 —— **推送这一侧对使用者是零门槛的**,
P4 给朋友用时他们不需要装任何新 App。

> P3 的审批卡片带按钮,**点按钮很可能会跳回企业微信**(以实际体验为准)。
> 也就是说"只读"的那部分在微信里,"要动手"的那部分在企微里 —— 这个分工
> 和产品设计本来的意图是一致的:审批本来就该是个需要你专门去做一下的动作。
