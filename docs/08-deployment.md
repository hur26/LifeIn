...
1. 打开 App,点"扫码配置"扫 5.1 生成的那个 `device-<设备名>.html`;
   不想给相机权限就把那串配码粘进下面的输入框 —— 两条路都行
...
**只有 [03 的四条硬门槛](03-roadmap.md#前置硬门槛不满足则不开放)全过了才走到这里。**
那四条不是流程,是**不满足则不开放** —— 尤其"备份恢复演练做过至少一次",
现在 [`scripts/restore-drill.md`](../scripts/restore-drill.md) 那张记录表还是空的。

### 7.1 一条命令对一遍

```bash
python -m lifein.admin check-user --user <uuid>
```

它替代的是"我记得都配好了"。每一条不通过都会说清**那样东西不在时他会看到什么** ——
"缺采集白名单"对配置的人没有信息量,"他的通知全被丢掉,而且不会报错"才有。

最后几条是代码查不了的(他有没有读过隐私说明、知不知道群消息会被采),
**但每次都会被念一遍**。

### 7.2 企微要把他加进可见范围

企微那条通道按 `users.wecom_userid` 逐个发,所以多成员本身不用改代码。
但有一件事**加了人也不生效**:

> 企微后台 → 应用管理 → 你那个自建应用 → **可见范围**,把他加进去。

不加的话 `message/send` 会回一个"不在可见范围"的错误码,
而降级链会**静默地退到邮件** —— 表现是"他一直收不到企微消息,
但邮件里都有",而那个现象很容易被当成"他没装企微"。

### 7.3 他不装 App 的话会少什么

[01 §4 的降级矩阵](01-product-spec.md#能力降级矩阵)那一行摊开:

| 少了什么 | 具体表现 |
| --- | --- |
| **记账** | 退化为月度导出 —— 每月他自己导一次账单发给你,没有实时入账,也没有超支当天的预警 |
| **群消息摘要** | 完全没有。微信消息只存在于手机上,服务端够不着 |
| **待办与日程** | 只能靠消息推给他,**勾不掉** —— 没有列表、没有小组件、不写系统日历 |
| 每日摘要 / 问答 / 审批 | 照常。这几样只要有一条消息通道就成立 |

**iOS 那一行不是排期问题,是平台限制** —— iOS 没有通知监听的等价能力。

所以接一个 iOS 朋友之前要先说清楚:他拿到的是上面第四行,
而**这个产品最有价值的那部分(实时记账、群摘要)他用不上**。
说在前面和用了两周之后才发现,是完全不同的两件事。
...
控制台从 2026-09-10 起分两层
([ADR-029](04-tech-decisions.md#adr-029--控制台分成用户层与运营层运营层加一个口令认证面))。
**用户层不用配任何东西就能用,运营层要配一格才存在。**

### 8.1 用户层:什么都不用配

朋友在 App 里点 **状态 → 在浏览器打开**,就进去了。那条链接十五分钟有效、
不需要登录。他在上面能做的:导出数据、删掉已采的数据、加设备、吊销设备、
开关采集、改放行的来源、读隐私说明。

**唯一的前置条件是 `PUBLIC_BASE_URL` 配了、而且是 https。**
两层的会话 cookie 都带 `Secure`,浏览器在明文上根本不会存它 ——
表现是"点进去又跳回去",而不是报错。

配码那一页还额外要它:二维码里写的就是这个地址
([07 §2.1](07-config.md) 那段"不能从请求里推")。

### 8.2 运营层:配一格,或者不配

```bash
python -m lifein.admin console-password
# 当场读一次口令,打出一行 CONSOLE_ADMIN_PASSWORD_HASH=scrypt$…
# 它不写任何文件 —— 口令本身哪儿都不存
```

把那一行放进 `.env`,重启。之后 `https://你的域名/admin` 是运营台。

**留空是一个正经的选择。** 一个人自己用的时候运营层没有存在的必要,
而**不存在的认证面是攻不破的** —— 留空时 `/admin` 只有一句"这台机器上
没有运营台",没有登录框,POST 过去也不会发出任何 cookie。

顺便标一下运营者自己那一行,好让运营台上"切回用户版"知道切到哪:

```bash
python -m lifein.admin set-admin --user <你自己的 uuid>
```

**这一列不是权限位**([06 §2.10](06-data-model.md)):标了不会让谁登得进
运营台,不标也不影响登录 —— 只是那个切换按钮不显示。

### 8.3 运营台上有什么,以及刻意没有什么

| 有 | 回答的问题 |
| --- | --- |
| 总览 | 所有人加起来,现在有没有事(掉线、待确认积压、模型开销) |
| 用户 | 谁是谁、开着还是停了;停用 / 恢复 / 标运营者 |
| 用户详情 | 他的设备、白名单(只读)、最近的任务运行 |
| 运维 | 失败的任务窗口、卡住的审批、还停在旧主密钥上的凭据 |

**三样刻意留在终端里**,不要因为"顺手"就加进网页:

- `admin purge-user` —— 它擦掉一个人的全部存在,不可撤销。
  **一个能擦掉一个人的按钮,该要求你先打开终端**
- `admin export`(别人的数据)—— 本人自己点一下就有;运营者替他导
  是一次需要解释的动作
- `set-imap` / `login-weixin` —— 授权码一律从交互输入读,
  而一个 HTTP 表单会进反代日志
...
| 做什么 | 该看到什么 |
| --- | --- |
| App 里点"在浏览器打开" | 浏览器打开总览,地址栏里**没有** `?t=` —— token 只在门口出现一次 |
| 在控制台点"添加设备" | 一张二维码,底下那串以 `{"v": 2` 开头(里面没有密钥) |
| 打开 `/admin`(没配口令) | "这台机器上没有运营台" |
| 打开 `/admin`(配了口令) | 一个口令框,**没有侧边栏** —— 还没进门的人不该看见这台机器上有什么 |
| 故意输错五次 | "失败太多次了,15 分钟之后再试",这时**输对也进不去** |
| 登进去点"切回用户版" | 跳到你自己账号的用户层 |

**最后一条是最容易出错的那个**:如果它跳到了别人的账号,
说明 `is_admin` 标错了行 —— 在"用户"里看一眼哪一行挂着"运营者"角标。
