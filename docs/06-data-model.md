...
#### 后四条是补上去的,每一条都对应一种会真发生的错

前四条(金额、5 分钟、跨渠道、卡号)原来就有,但它们**只描述了"像不像同一笔"**,
没有描述"是不是同一件事":

| 加的条件 | 不加会怎样 |
| --- | --- |
| `currency` 相等 | 50 USD 的消费并进 50 CNY 那笔,总额少掉一笔外币消费 |
| `direction` 相等 | **退款并进支出。** 一笔 38.5 的退款短信和 38.5 的消费通知差几分钟到,合并之后账本上既没有那笔支出的对账价值,也没有那笔退款 |
| `kind` 相等 | 信用卡还款并进同额消费。还款本来就不进统计,并进去之后那笔消费的 `kind` 也说不清了 |
| 候选唯一 | 原来是 `ORDER BY 时间差 LIMIT 1` —— **两笔都匹配时它悄悄挑一个**。而"两张卡里各有一笔同额"正是最需要人来看的情况 |

**候选不唯一时选择不合并。** 结果是账本上多一笔,而 06 一贯的取舍是
"少了一笔比多了一笔更难发现" —— 多出来的那笔你打开账本就看得见,
少掉的那笔要到月底才觉得"这个月怎么花得少"。
...
上面那段假定实时那笔先入账。**顺序反过来时原来会记两笔**,而这不是罕见情况:

- 头一次接入时先手动导一份历史对账单,采集器随后才开始工作
- 补跑对账 job,而当天晚些时候那笔消费的短信才被采到
- 卡刚绑上,而这个月的账单里已经有它

对账补录出来的那一笔 `stage='reconciled'`,而
[`repos/transactions.record`](../lifein/repos/transactions.py) 对 `reconciled`
**故意跳过 5 分钟合并**(理由见那段注释:对账那一遍已经用更强的规则判过了)。
于是晚上那条支付宝/银行通知走实时那一路,拿 5 分钟窗口去比 —— 而对账单上
那个日期是入账日,差着几天,永远比不上。两笔各记一次。

所以实时入账多一道,**在 5 分钟合并落空之后**才走:

```
新的一笔 stage='realtime',5 分钟跨渠道合并没找到:
  再找一条"还没有人认领的对账单行":
      同一 user_id AND amount / currency / direction / kind 相等
        AND account_hint 相等或一方为空
        AND |occurred_at 差| <= 对账时间窗(和上面同一个 3 天)
        AND stage = 'reconciled'
        AND cardinality(merged_from_event_ids) = 0   -- 还没被认领过
        AND 候选唯一
    → 认领:event_id 进 merged_from_event_ids,
      **occurred_at 改成实时那条的时间**,不新建
    → 没找到:正常新建
```

**只有这一处会改已有行的 `occurred_at`,理由要写清楚:** 对账单给的是入账日,
实时通知给的是消费当时。两阶段入账里"时间以实时为准、商户以对账单为准"
([ADR-012](04-tech-decisions.md#adr-012--记账采用两阶段) 那张表)—— 而这一次
先落地的恰好是权威性较低的那一半,所以补的时候要把它换回来。
不换的话一笔周六晚上的消费会停在周一,而月度报表按天切。

`cardinality(merged_from_event_ids) = 0` 是这一路的幂等键:认领过一次之后
第二条通知再来只会走 5 分钟那一路(那时 `occurred_at` 已经是消费当时了,
比得上),而不会把第三条、第四条一起吞进来。
...
pending ──同意──→ approved ──认领──→ executing ──┬─→ executed
             │         │                          └─→ failed
             │         └──「拒绝 12」──→ rejected      ← 撤回,见下
             └──「拒绝 12」───────────→ rejected
```

#### `approved → rejected`:打错数字之后唯一的补救

审批走的是文字(`同意 12`,[ADR-027](04-tech-decisions.md#adr-027--审批用文字回复不做交互卡片)),
而**文字方案最真实的风险是打错数字** —— "同意 12" 敲成 "同意 21" 会同意
另外一条,而 L3 是不可撤销的。

回复的确认卡片会**把内容原样回显**("替你发一条消息:今晚饭局我去不了了"),
所以打错的那一刻他就看得见。但看得见不等于停得下来:
原来 `rejected` 只能从 `pending` 来,于是他只会收到一句"已经同意过了,正在做"。

所以 `approved` 也能被拒绝 —— **只要执行还没认领**。
撤回和认领(`approved → executing`)争的是同一行,而两条都带着
`WHERE status = 'approved'`,所以**恰好一个会赢**:
要么执行开始了(那时撤不回来,如实说),要么撤回成功了(那时一个字都没发出去)。
...
5. **交易通知的正文送出去之前要遮掉卡号和余额。** 见下。

### 第 4 条原来只做了一半

"抽成结构化行单独给"做到了,**但正文照样原样送出去** ——
而一条银行短信的正文长这样:
...
于是卡号和余额一起去了外部供应商那里,尽管:

- 记账 agent 这一轮**只回答"这是哪一类资金变动"**,卡号和余额都用不上
- 卡号已经作为 `account_hint` 单独抠出来了(而且**没有**放进给模型的字段里,
  因为它判断不需要)
- 余额是这条短信里最敏感的一个数字。**它甚至不在我们要存的清单里** ——
  [R10](05-risks.md#r10--手机端采集器的越权读取) 说交易类只保留
  "金额、时间、卡号后四位、商户"
...
| 遮什么 | 遮成 | 为什么 |
| --- | --- | --- |
| `余额` / `可用额度` 后面的数字 | `余额****` | 判断类型用不上,而它是最敏感的那个数字 |
| `尾号` / `卡号` / `账号` 后面的数字 | `尾号****` | 已经作为结构化字段抠出来了,而模型判断不需要它 |
| 连续 6 位以上的数字串 | `****` | 卡号、账号、手机号。**金额不会被误伤**:它带小数点或"元" |

**金额不遮。** 它是这条通知的主语,遮掉之后模型判不出"消费"和"待支付提醒"
的区别 —— 而那正是四层防误判要它做的事。它本来也在结构化字段里给了一份。

**遮的是送出去的那一份,不是库里那一份。**
第 4 层复核要拿原文做"金额逐字比对",而那一步读的是 `raw["parsed"]["matched"]`
和 `normalized.body` —— 两者都不经过这道遮罩。
...
| `sender` | sms | **通知上显示的发件人,不保证是号码**(见下)。`sms_sender` 规则按它匹配 |
...
> **`sender` 曾经写的是"发件号码",而那个东西不存在**([ADR-034](04-tech-decisions.md#adr-034--银行短信按短信签名匹配不按发件人))。
> App 走的是通知监听(没有 `READ_SMS`),拿到的是**系统渲染给人看的标题** ——
> 2026-09-11 的三条真实短信里,两条是显示名(`招商银行`、`游戏中心`),
> 一条是网关号码(`10693495555`)。
>
> **银行改按 `sms_signature` 匹配**:正文开头 `【…】` 里的机构名,由发信方
> 写进内容,不受厂商显示逻辑与网关号码影响。**签名由两端各自从正文里提取,
> 不作为字段上报** —— 传一个"设备算好的签名"会让[铁律 11](../AGENTS.md#1-铁律)
> 那两道独立过滤退化成一道。
...
> 这样服务端加一类待确认不必等 App 发版,而没发版的 App
> 也不会拿错误的形状去确认。
...
**这张表是 P2 补上的。** 原来服务端只会写 `todos`,别的一律 422,
注释还写着"P2 的账目进来时再接"—— 而 P2 的代码早就在往队列里塞
`target_table='transactions'` 了。结果是**低置信度的那些钱堆在队列里,
永远入不了账**:App 列得出来,点确认拿一个 422,而队列三十天后过期。

| `target_table` | 确认后走哪个工具 | 客户端能改的字段 |
| --- | --- | --- |
| `todos` | `todo.create`(L2) | `title` / `notes` / `starts_at` / `ends_at` |
| `transactions` | `txn.record`(L2) | `category` / `kind` / `merchant_raw` |
| 其它 | **422,并且不认领那一条** | —— |

不认识的目标表回 422 而不是硬写:那一条仍然是 `pending`,
将来版本更新了还能处理。**认领了再失败等于把它变成永远处理不了的那种。**

#### 账目那一行为什么不让改金额

`transactions` 的可改字段里**没有 `amount`、`direction`、`occurred_at`**,
这不是漏了:

- 那三项是规则从原文里抠出来的,不是模型说的([铁律 9](../AGENTS.md#1-铁律))。
  进待确认的原因是"这是不是一笔支出说不准",不是"钱数说不准"
- 放开金额等于**给了客户端一条往账本里写任意数字的路**,
  而那一行的出处还写着"来自那条短信"—— 比记错一笔更糟的是记错了还查不出来
- 金额真的抠错了,正确的动作是**拒绝**它,然后手动补一笔(§6.12):
  那一笔的出处是"用户自己填的",账本上分得清
...
1. **写入目标表与更新 `status` 在同一个事务里**,由 `pending.confirm(writer=…)`
   ...
   不会出现"确认了但没写进去"
2. **客户端只能改上表列出的那几项。** `provenance`、`created_by_agent`、
   `source_event_id` 一律沿用队列里那份 —— 出处不由客户端说了算(铁律 5)
...
**只有这一条。** 这一节原来还列着 `POST /app/ledger/receipts`("上传小票照片,
回一份待确认的解析结果"),而**那条接口不存在,也不该存在** ——
见下面"小票不走服务端"。
...
#### 小票不走服务端,它只是把这张表单先填上几格

[ADR-021 的 2026-09 补充](04-tech-decisions.md#adr-021--安卓端依赖清单逐条定性)
把小票识别挪到了手机上,理由是一条:**照片里有的东西比一笔账多得多** ——
同行的人、邻桌、你的手。服务端从来没收到过的东西,不需要承诺"不留"。

这一节此前没跟着改,于是留下两句已经不成立的话:

| 原来写的 | 为什么不再成立 |
| --- | --- |
| `POST /app/ledger/receipts` 上传照片 | 照片不上传了,这条接口没有实现,也不会有 |
| "小票识别的结果**一律进待确认**" | 待确认是给"没有人在看"的判定准备的,而这里人就在屏幕前 |

**第二句是这次改动里真正要说清的一条。** 待确认队列存在的理由是
*agent 拿不准,而当时没有人可以问*([§2.7](#27-pending_confirmations--统一待确认队列))。
小票的情形反过来:识别刚发生在他手上,结果**就显示在他面前的表单里**,
金额抠错了他当场就改。这时再塞进待确认,是让他**为同一笔账点两次头** ——
先在表单上确认一次,再去另一个页面确认一次。

> 那条"误记率 = 0"仍然成立,只是**由谁保证换了人**:
> 不是待确认队列,是屏幕前那个正在看着识别结果的人。
> 而他确认之后落下来的那一笔,出处是 `trust=user_input` —— 账本上分得清
> 哪些是他自己填的,哪些是 agent 记的。

所以这条链路上服务端一个字节的照片都没见过:

```
手机:拍照 → ML Kit 中文 OCR → 抠出金额/商户 → 填进"手动补一笔"的表单
      ↓ 人核对、可修改
POST /app/ledger/transactions        ← 和纯手填走的是同一条路
```

**识别不准不是错误,是常态**(小票上的"实收"是收了多少现金,不是这笔花了多少 ——
这一条真踩过)。所以 App 上那句提示写的是"认出来了,核对一下再记上",
而不是"已识别"。

### 6.13 Web 控制台 `/console`(P4)

**它不是第二个 App。** [03 的 P4](03-roadmap.md#p4--多用户托管) 说
"移动端查看已由 App 覆盖,**Web 只剩这些 P4 的合规必需项**" ——
账本、待办、记忆一个都不进来,一份两处实现的界面会有两套 bug 和两次要改。

| 方法 | 路径 | 认证 | 说明 |
| --- | --- | --- | --- |
| POST | `/app/console/link` | App 的 Bearer token | 换一张十五分钟的一次性链接 |
| GET | `/console?t=…` | 那张 token | **认出人之后换 cookie,303 跳到干净的 `/console`** |
| GET | `/console` | 会话 cookie | 首页:设备列表、导出、添加设备 |
| POST | `/console/devices/invite` | 会话 cookie | **出一张配码二维码** |
| POST | `/console/export` | 会话 cookie | 下载导出(JSON,不含凭据) |
| GET | `/console/privacy` | **不需要** | 隐私说明 |

认证那一套(为什么是一次性链接、为什么 token 只在门口出现一次、
为什么导出是 POST)写在 [§2.14](#214-console_links--控制台的一次性链接)。

#### 添加设备:这一条是 P4 那个问题的答案

03 的 P4 写着朋友侧配码要做成"Web 控制台里点添加设备 → 页面显示二维码",
而**理由是那句"现在的配码要人在服务器上跑 `issue-device`,非技术背景的人
做不到"**。二维码的内容在 P4 第 1 片已经修对了(里面是一次性换取码,
不是密钥),但出码这个动作还在终端里 —— 这一节把它搬进浏览器。

```
POST /console/devices/invite
  → 200 HTML:二维码 + 明文串 + 有效期
```

四条:

1. **POST,不是链接。** 它签发的是一张能换走两把密钥的码 —— 而 GET 会被
   浏览器预取、被"重新打开上次的标签页"重放。**每重放一次就多一张有效的码**,
   而每一张都能配上一台设备
2. **码只在这一页出现这一次。** 库里存的是 sha256,服务端自己也说不出它是什么
3. **二维码是内联 SVG,不是 `<img src>`。** 这个控制台里没有任何外部引用 ——
   多一个外链就多一处 Referer 会带着东西出去的地方
4. **配出来的码只能配到点它的那个人自己的账号上。** 控制台会话是他的 App
   换出来的,`user_id` 从会话里取,页面上没有任何地方能指定别人。
   **给朋友开账号是另一件事**,那要在服务器上跑 `admin add-user`

#### 吊销还在命令行里

控制台只**列**设备(设备名、用途、有效/已吊销),吊销要跑
`admin revoke-device`。这不是遗漏,是一次划界:

- 添加设备是**朋友会做的事**,一个人一台新手机就要做一次
- 吊销是**手机丢了那天做的事**,而那天他会找白杨 —— 而且那时该做的不止
  吊销一台(见 [09 §7](09-privacy.md))

> 哪天"手机丢了"变成一件需要自助处理的事,再把它搬进来。
> 在那之前,一个平时用不到、真用到时不敢按的按钮,不如没有。

### 6.14 日历采集 `POST /ingest/calendar`

**日历数据源换人了。** 此前唯一的一条是企微日程,而企微整个退出了
([ADR-026](04-tech-decisions.md#adr-026--企业微信整个退出消息面只剩-ilink-与邮件))
—— 这一节是它的替代:**从手机的系统日历读**。

换过来不只是"找一个替代品",它严格更好:系统日历里有企微同步过来的、
飞书的、订阅的、手动建的**全部**日程,而企微那条只看得见企微自己那一个。
App 本来就有 `READ_CALENDAR`(它要往里写日程,[ADR-020](04-tech-decisions.md#adr-020--待办与日程落在自己的-app企微退出主链路)),
所以这条路上没有新增任何权限。

```
POST /ingest/calendar          ← 采集凭据签名,和 §6.4 同一套
{
  "device_id": "android-…",
  "events": [{
    "event_id": "gmail.com/8f3a…",   ← 见下面"去重键"
    "calendar": "白杨的日历",
    "title": "周三下午三点评审",
...
→ 200 {"accepted": 3, "duplicates": 12, "dropped": {"malformed": 0, "loopback": 2, …}}
```

#### 去重键:`_ID` 靠不住

`CalendarContract.Events._ID` 是**本机自增**的 —— 换手机、重装、清数据之后
同一场会议会拿到一个新号,于是它会作为一条新事件再进来一次。
而 `_SYNC_ID`(同步 id)跨设备稳定,但**本地建的日程没有**。

所以设备端按这个顺序拼:

```
event_id = "<日历账户名>/<_SYNC_ID 或 _ID>"
```

账户名前缀不能省:两个日历各自的 `_ID` 从 1 开始,不带前缀的话
"公司日历的第 5 条"和"个人日历的第 5 条"是同一个键。

> **这条键最坏的失效方式是"重装之后所有日程重新进一遍"** ——
> 表现是某天的摘要里每件事都出现两次,而两条的内容一模一样。
> 用 `_SYNC_ID` 挡住了绝大多数,剩下的(纯本地日程)只能认。

#### 回环:自己写进去的不能再读回来

**这是这一片最容易出的事故。** [ADR-020](04-tech-decisions.md#adr-020--待办与日程落在自己的-app企微退出主链路)
说日程提取出来之后由 App 写进系统日历。如果读的时候不排除它们:

```
邮件里提到"周三三点开会" → 提取 → todos → App 写进系统日历
                                              ↓
                          日历采集读到它 → raw_events → 提取 agent 又看见
                                              ↓
                                    再建一条 todo → 再写进日历 → ……
```

一条日程会**指数级地繁殖**,而每一轮都长得像真的。

挡它的要有**两层,而且第二层不能依赖本机的任何状态**:

| 层 | 是什么 | 什么时候救场 |
| --- | --- | --- |
| 1 | 本地 `calendar_links` 表里那些事件 id | 日常。快、准 |
| 2 | **事件自己身上的标记** | 第 1 层丢了的时候 |

**第一版只做了第 1 层,而那是个 bug。** 那张表在这些时候会丢或者对不上:

- 用户在系统设置里「清除数据」、恢复出厂、换手机重装
- `insert()` 成功之后、写本地表之前进程被杀
- 删日历事件失败,而本地表那一行照样被删掉了

**丢了的后果不是"少挡一次",是永久性的**:那些事件从此对过滤器不可见,
每一轮采集都会把它们读回来,再提取成新的待办,再写进日历 —— 而这正是
上面那个繁殖循环。

所以写日历时在事件上留两个印:

- `Events.CUSTOM_APP_PACKAGE` = App 的包名。**这一列就是为这件事设计的**
- 描述末尾追加一行 `由 LifeIn 写入`。它土,但它**跟着同步走** ——
  `CUSTOM_APP_PACKAGE` 在某些同步适配器和 ROM 上会被丢掉,而描述不会

> **两个印都留,是因为它们的失效方式不重叠。** 只留前者,某些机型上等于没留;
> 只留后者,用户手动改了描述就没了。而这道防线**丢一次就永久失效**,
> 值得多花一列。

被滤掉的计进设备端日志,那个数**不该长期是 0**:它是这道防线还活着的
唯一证据。

**过滤在设备端做**,不是等服务端去查 —— 服务端要按 id 反查一遍 `todos`,
而设备端手上本来就有那张表,而且第 2 层那个印只有读日历时才看得见。

#### 只读勾选过的日历

手机上的日历常常有一堆:生日、节假日、订阅的球赛、公司的全员会。
全读会把摘要淹掉,而**淹掉的摘要等于没有摘要**。

所以**默认一个都不读**,由用户在 App 的状态页里勾。这和采集白名单
([R10](05-risks.md#r10--手机端采集器的越权读取))是同一条思路:
默认拒绝,用户显式放行,而且随时能改。

#### 时间窗:前一天到后三十天

再往前读会把几年的历史一次性灌进来(而摘要只关心最近);再往后读,
那些事在发生之前还会改。前一天是留给"昨天那场会你没去成"这类事的。

**窗口在设备端切**,和上报批量上限(§6.4 的 `MAX_BATCH`)同一个理由:
分批的责任在设备端。

#### `trust` 由谁建的决定

自己建的日程 `user_input`,别人邀请的 `external` ——
后者的标题和描述是别人写的,**和一封邮件没有区别**(R3)。
设备端拿 `Events.ORGANIZER` 和日历的 `OWNER_ACCOUNT` 比,比出来的结果放在
`self_organized` 里;服务端不重算,因为它不知道这台手机上哪个账户是"本人"。
...
### 6.16 副驾分析 `POST /app/copilot/analyze`

[ADR-035](04-tech-decisions.md#adr-035--副驾单开一条无障碍读屏通道与通知监听并存)
与 [ADR-037](04-tech-decisions.md#adr-037--副驾走判断--起草--排序三次调用只有一次能产出自由文本) 的落点。
走查询端凭据(bearer token,§6.3),**不是采集端** ——
它是一次问答,不是一次上报。
...
```json
{
  "device_id": "android-1a2b3c4d5e6f",
  "app": "wechat",
  "title": "张三",
  "messages": [
    {"side": "other", "text": "你昨天说的那个事到底怎么样了"},
    {"side": "me",    "text": "我下午给你回"}
  ],
  "history": [
    {"side": "other", "text": "周末有空吗"}
  ],
  "capture_note": "ocr"
}
```
...
| 字段 | 说明 |
| --- | --- |
| `app` | 放行的聊天 App 标识,服务端只认已知值,不认的直接 422 |
| `title` | 会话标题。**用来对 `entity_aliases`**,对不上就当陌生人,不报错 |
| `messages` | **当前屏幕上**那段对话,最多 40 条,按时间升序 |
| `history` | 本地历史里超出屏幕的那部分,最多 30 条,可省。来自手机本地库([ADR-038](04-tech-decisions.md#adr-038--副驾的对话历史存在手机本地不进服务端库)) |
| `side` | 只有 `me` / `other` 两个值。**服务端不猜** —— 猜错比不知道更糟 |
| `capture_note` | 采集方式的说明(如 `ocr` = 走了截屏识别)。会原样回给客户端提示用户 |
...
{
  "judgement": {
    "literal": false, "true_intent": "seek_explanation", "intent_confidence": 0.71,
    "danger_level": 4, "needs": "explanation", "best_action": "explain",
    "should_reply_now": true, "tension_resolved": false
  },
  "candidates": [
    {"text": "抱歉拖到现在,我今晚给你个准话", "rank": 1, "share": 0.52},
    {"text": "这两天卡在别的事上了,我明早处理", "rank": 2, "share": 0.31},
...
1. **`candidates` 恒为 3 条。** 模型给不满就补一条保守的,并把
   `degraded` 置成 `"draft_short"` —— 客户端按 3 条排版,给 2 条会让界面错位
2. **每条 `text` ≤ 40 字。** 这是 [R3](05-risks.md#r3--提示注入不可信的外部内容) 的防线之一,
   不是省 token:候选要短到人真的会读完再发
3. **请求体不写任何表。** 唯一落库的是 `tool_calls` 那条审计,
   而它只有字段名与长度,没有内容(§2.9)。这是 ADR-038 那句
   "副驾读记忆,不写记忆"在接口层的形式
4. **必须过 `quota.guard`。** 一次请求打三次模型,是全系统单次成本最高的入口。
   超额返回 `degraded: "quota"` 且 `candidates: []`,**不是 500** ——
   用户要看到"这个月额度用完了",不是一个红叉
5. **坏行丢弃,不 422 整批**(和 §6.4 同一条规矩)。
   但 `messages` 全空是 422:那说明客户端根本没读到东西,不该发这个请求
6. **`judgement` 里全是枚举值与分数,不含自由文本。** 三次模型调用里
   只有起草那次能产出会进输入框的文本(ADR-037)——
   这条形状约束就是那个结构性收益的落点,**改它之前先读 ADR-037**
7. **服务端不知道用户选了哪条,也不知道有没有发出去。**
   填入发生在手机上,发送发生在微信里。**这条接口到此为止**

#### 它和治理层的关系:自己不是工具,但要过网关去查记忆

这条接口本身不是一个注册工具,和 §6.6 那条"App 的写操作不过网关"不同 ——
那一条是因为**写的是自己的地盘**;这一条是因为**它压根没写任何东西**。

但它要查记忆:`memory.search_entities` 用 `title` 对联系人、
`memory.recall_facts` 取相关事实。这三个都是 L1 只读工具,
**照常走 `Gateway.call()`**,照常进 `tool_calls`。
副驾的 agent 白名单里**一个 L2、L3 都没有** ——
所以就算提示注入成功,它能做的最坏的事也只是给出一条烂建议,
而那条建议还要经过你的眼睛和你的手指。

这也是为什么 `approvals` 一行都不会写:**没有 L3,就没有审批。**
[铁律 8](../AGENTS.md#1-铁律) 说的是"L3 永远不由外部内容直接触发",
而这里根本没有 L3 —— 那条 CHECK 碰都碰不到。
...
- ~~**审批卡片的回调**~~ ✅ 已定:**没有回调,回一句`同意 12`** —— iLink 协议里不存在交互卡片([ADR-027](04-tech-decisions.md#adr-027--审批用文字回复不做交互卡片))
- ~~**小票识别放哪一端**~~ ✅ 已定:手机上识别,照片不上传(ADR-021 的 2026-09 补充)
- ~~**评测集格式**(agent 契约第 5 项)~~ ✅ 已定,见 §7
- ~~**副驾怎么把屏幕上的对话交上来**~~ ✅ 已定,见 §6.16
- **副驾的本地库没有契约。** 对话历史存在手机上(ADR-038),
  服务端看不见也管不着,所以它不在这份文档里 ——
  但**换机丢历史、上限 300 条**这两条要在 App 里对用户说清楚,
  而不是只写在 ADR 里
