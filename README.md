...
它读你的邮箱、日历、手机通知和账单,把散落各处的生活信息整理成长期记忆,<br/>
...
一个跑在你自己服务器上的程序,加一个装在你手机上的安卓 App。

服务端定时读取邮箱、日历和笔记;App 监听手机上的交易通知与微信群消息 ——
**那些东西只存在于手机上,服务端够不着**。两边的数据归一成结构化的生活事件流,
提炼成能长期积累的记忆。

看和操作在 App 里(账本、报表、待确认队列);推送和审批走企业微信
(每日摘要、主动提醒、"要不要替你发这条消息"的确认卡片)。
**App 不接收任何推送**,因此不需要接厂商推送通道。

P5 起 App 上多一个**副驾**:你在微信或 QQ 里聊天时,它读你当前打开的那个聊天窗、
告诉你对方大概想要什么、给三条候选回复,点一下填进输入框。
**发送键永远由你按** —— 它不替你发,一次都不会。

**它不是**:通用 agent 框架、给陌生人用的商业产品、又一个聊天机器人。
副驾也不是聊天机器人 —— **它不和你聊,它帮你和别人聊**。
...
| 🔨 | **P5** | 副驾:读当前聊天窗 → 候选回复 → 填进输入框 | 两周内你真的用出去 20 条 | 3–4 周 |
...
| [09 隐私说明](docs/09-privacy.md) | **给要接入的朋友看的** —— 拿到什么、存在哪、白杨能看到什么、你自己能怎么关和删 |

新加入的人建议按 **01 → 03 → 04** 的顺序读;要动手写代码,先读 [AGENTS.md](AGENTS.md) 再读 06。
