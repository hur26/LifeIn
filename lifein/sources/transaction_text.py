...
**千分位那一支里的 `+` 不能写成 `*`。** 写成 `*` 时它不要求真的有逗号,
于是 `人民币5000.00` 会先匹配掉前三位就收工 —— 解出来是 **500**,
而 `人民币12345.67元` 解出来是 **123**。

带 `元` 后缀的那一支侥幸躲过了:它要回溯去匹配 `元`,所以会退到第二个分支。
**而货币前缀那一支后面没有任何锚,没有回溯压力。**

这个洞活到 2026-09-11 才被发现,因为在此之前所有用例的金额都在三位以内
(70.00、38.50、10.00)—— 直到一批真实短信里出现了一笔 5000 的工资入账。
"""

_CARD_WORDS = "尾号|卡号|账号|账户|末四位|尾数|储蓄卡|信用卡|借记卡|银行卡|储值卡|工资卡"
"""卡号前面可能出现的词。
...
**`账户` 是 2026-09-11 从 147 条真实银行短信里数出来补的**(它出现 10 次,
是词表唯一漏掉的常用说法)。漏掉它有两个后果,第二个更要紧:

1. `account_hint` 抠不到 —— 对账少一维
2. **那个卡号原样进了送给外部模型的那一份**。`_MASK_ACCOUNT` 用的是同一个
   词表,词表漏了它就不会被打码([R12](../../docs/05-risks.md))

一个词表两处用,是刻意的:补一个词,抽取和脱敏同时跟上,不会只修一半。
"""
...
_INBOUND = ("收入", "转入", "到账", "收款", "退款", "退回", "存入", "入账")
"""**`入账` 是 2026-09-11 从真实短信里补上的。**

在那之前"您账户0361入账工资,人民币5000.00"两边都不命中,
于是落到"认不出来按流出"那条默认 —— **一笔工资被记成了支出**。

默认按流出本身没错(多记一笔支出你会在账本上看见并改掉,漏记不会有人提醒),
但那条默认是给"真认不出来"的,不是给"词表漏了一个常用词"的。
"""
