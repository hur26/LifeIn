...
挡它的是 `todos.device_ref`:那一列存的就是 App 写进去的那些事件 id。
**设备端在上报之前就过滤掉**,不是等服务端去查 —— 服务端要按 id 反查一遍
`todos`,而设备端手上本来就有那张表(Room 里的日历幂等表)。
被滤掉的计进 `dropped.loopback`,那个数**不该长期是 0**:它是这道防线
还活着的唯一证据。
