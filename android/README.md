...
2. 打开 App,**扫那个二维码**(`device-<设备名>.html`),或者把配码粘进配置页。
   **只显示一次** —— 相机权限点了"扫码配置"才会要,不给也能用(粘贴那条路一直在)
...
- **配码二维码里是明文的两把密钥**。自己从屏幕上扫、当场用掉没问题;
  但 P4 把它发给朋友时会走微信 —— 那等于把密钥发在聊天里,
  那时要改成"二维码里只放一次性换取码"([ADR-021](../docs/04-tech-decisions.md#adr-021--安卓端依赖清单逐条定性) 的改判那节)
